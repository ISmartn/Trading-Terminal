...
You can use the basic dashboard without broker credentials. Add a free Upstox OAuth access token only if you want the best option-chain, Greeks, and live tick experience.
...
| **🔑 Broker API Keys** | Connect your Upstox account for live data (BYOK) | ✅ Working (Upstox fully connected) |
| **📡 Live Feed** | Real-time index/VIX LTP via Upstox Market Quote API | ✅ Working |
| **🗄️ Local Database** | IndexedDB-based persistence for price snapshots and candle history | ✅ Working |
| **📥 Chart Downloader** | Batch download OHLCV candles via Upstox historical API | ✅ Working (requires Upstox token) |
...
|-------------------|-------------------------|
| Dashboard overview, index cards, watchlist, sector flow, scanners, and many fallback data widgets | Full Upstox option chain, Greeks, expiry metadata, and live LTP feed |
| Local settings, local watchlist, browser storage, and theme preferences | Highest-quality real-time derivatives data |
| Upstox historical candle downloads | Broker-specific authenticated endpoints |

### Step 4 (Optional): Add Upstox API for Premium Data

For the best experience (real-time option chain, Greeks, live index/VIX ticks):

1. **Create an Upstox account** at [upstox.com](https://upstox.com)
2. **Create a developer app** at [Upstox Developer Portal](https://upstox.com/developer/api-documentation/open-api/)
3. **Complete OAuth login** to obtain an access token — see [Authentication docs](https://upstox.com/developer/api-documentation/authentication/)
...
```env
UPSTOX_ACCESS_TOKEN=your_oauth_access_token_here
```

6. **Restart the app** (press `Ctrl+C` to stop, then run `npm run dev` again)

The proxy server will automatically detect your Upstox credentials and start the live LTP feed.

> **📝 Note:** You can also add Upstox credentials from the UI — go to **Broker Settings** (`/broker-settings`), select **Upstox**, and enter your OAuth access token. Stored in browser localStorage only.

### Step 5 (Optional): Add Other Broker Keys

The Broker Settings page supports entering API keys for other Indian brokers:

| Broker | Status |
|--------|--------|
| **Upstox** | ✅ Fully integrated (Option Chain, Greeks, Live LTP) |
| **Zerodha (Kite)** | 🔧 UI ready, backend integration coming soon |
| **Angel One (SmartAPI)** | 🔧 UI ready, backend integration coming soon |
| **Dhan** | 🔧 Legacy UI slot (use Upstox for live data) |
...
```
Priority: Upstox API (1st) → NSE India (2nd) → TradingView (3rd)
```

| Source | What It Provides | Auth Needed? | Accuracy |
|--------|-----------------|-------------|----------|
| **Upstox API** ⭐ | Option Chain, Greeks, Expiry List, Live LTP Feed, Historical OHLCV | Yes (OAuth token) | Real-time |
