...
export default function DashboardLayout() {
  ...
  const hasUpstoxKeys = activeBroker?.brokerId === "upstox" && activeBroker.values.accessToken;
  ...
  useEffect(() => {
    ...
    console.info(
      "[SPOT-DEBUG] Enabled — watch for NIFTY/BANKNIFTY source lines; proxy: SPOT_FEED_DEBUG=1",
    );
    return startProxySpotMetricsPolling();
  }, []);
                ...
                <PopoverContent className="w-64 p-3" align="end">
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1.5 text-xs">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">Upstox API Keys</span>
                        <Badge variant="outline" className={`text-xs h-4 ${hasUpstoxKeys ? "border-bullish/50 text-bullish" : "border-warning/50 text-warning"}`}>
                          {hasUpstoxKeys ? "Configured" : "Not Set"}
                        </Badge>
                      </div>

                      <div className="flex items-center justify-between">
                        <span className="text-muted-foreground">Data Source</span>
                        <span className="font-medium">{isLiveData ? "Upstox / NSE / TradingView" : "Offline"}</span>
                      </div>
                    ...
                    {!isLiveData && (
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        Run <code className="bg-muted px-1 rounded text-xs">npm run dev</code> to start both Vite + proxy server, or configure Upstox API keys for live data.
                      </p>
                    )}
                  </div>
                </PopoverContent>
