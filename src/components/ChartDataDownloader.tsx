...
// Interval → Upstox API interval code
const INTERVALS = [
...
export function ChartDataDownloader() {
  ...
  const handleDownload = async () => {
    ...
    for (let i = 0; i < symbols.length; i++) {
      ...
      try {
        const instrumentKey = INSTRUMENT_KEYS[sym.symbol];
        if (!instrumentKey) {
          newResults.push({ symbol: sym.symbol, status: "skipped", candles: 0, error: "No Upstox instrument key" });
          setResults([...newResults]);
          continue;
        }

        const upstoxResult = await fetchHistoricalCandles(
          instrumentKey,
          sym.segment,
          sym.instrument,
          apiInterval,
        ...
        const rawData = upstoxResult?.data || upstoxResult;

        if (rawData?.close && Array.isArray(rawData.close) && rawData.close.filter((v: number | null) => v != null).length > 0) {
          ...
          newResults.push({ symbol: sym.symbol, status: "skipped", candles: 0, error: "No data from Upstox" });
        }
      } catch (err: any) {
