...
import { PROXY_BASE, getProxyPortLabel } from "@/lib/proxyConfig";
...
export default function OptionChain() {
  ...
  // ── Download Past Option Chain from Upstox API ──
  const downloadPastOC = useCallback(async (pastExpiry: string) => {
    setIsDownloadingPast(true);
    try {
      const res = await fetch(`${PROXY_BASE}/api/upstox-proxy?endpoint=option-chain&symbol=${symbol}&expiry=${pastExpiry}`, {
      ...
      if (chainData && typeof chainData === "object") {
        ...
        if (Array.isArray(chainData)) {
          chainData.forEach((row: any) => {
            ...
            rows.push([
              row.strike_price || row.strikePrice || "",
              ce.ltp || "",
              ceG.iv || "",
              ce.oi || "",
              ce.volume || "",
          ...
          if (typeof oc === "object" && !Array.isArray(oc)) {
            Object.entries(oc).forEach(([strike, legData]: [string, any]) => {
              rows.push([
                strike,
                legData?.ce?.ltp || legData?.ce?.last_price || "",
                legData?.ce?.iv || "",
                legData?.ce?.oi || "",
                legData?.ce?.volume || "",
                legData?.ce?.delta || "",
                legData?.ce?.bid_price || "",
                legData?.ce?.ask_price || "",
                legData?.pe?.ltp || legData?.pe?.last_price || "",
                legData?.pe?.iv || "",
                legData?.pe?.oi || "",
                legData?.pe?.volume || "",
                legData?.pe?.delta || "",
                legData?.pe?.bid_price || "",
                legData?.pe?.ask_price || "",
              ].join(","));
            });
          }
        }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-1.5">
          <Badge variant="outline" className={`gap-1 text-[11px] ${isLive ? "border-bullish/50 text-bullish" : afterHours ? "border-amber-500/50 text-amber-400" : "border-red-500/30 text-red-400"}`}>
            ...
            {isLive ? (data?.source === "upstox" ? "UPSTOX" : "NSE") : afterHours ? "CLOSED" : "OFFLINE"}
          </Badge>
      ...
      {!hasData && !afterHours && data !== undefined && (
        <div className="flex flex-col items-center justify-center py-16 text-muted-foreground border border-dashed border-border/50 rounded-lg bg-card/30">
          ...
          <p className="text-xs mt-1.5 max-w-sm text-center leading-relaxed">
            Check that the proxy server is running on port <code className="font-mono text-primary/70 bg-primary/10 px-1 rounded">{getProxyPortLabel()}</code> and Upstox credentials are configured in <code className="font-mono text-primary/70 bg-primary/10 px-1 rounded">.env</code>
          </p>
