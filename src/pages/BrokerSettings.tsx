...
import { testUpstoxConnection } from "@/lib/marketApi";
...
function ConnectionStatusPanel() {
  ...
  const [upstoxStatus, setUpstoxStatus] = useState<"idle" | "testing" | "success" | "error">("idle");
  const [upstoxMessage, setUpstoxMessage] = useState("");

  const handleTestUpstox = async () => {
    setUpstoxStatus("testing");
    try {
      const result = await testUpstoxConnection();
      if (result.status === "success") {
        setUpstoxStatus("success");
        setUpstoxMessage("Connected successfully");
        toast.success("Upstox API connection verified!");
      } else {
        setUpstoxStatus("error");
        setUpstoxMessage(result.message || "Connection failed");
        toast.error("Upstox connection failed: " + result.message);
      }
    } catch (e: any) {
      setUpstoxStatus("error");
      setUpstoxMessage(e.message || "Network error");
  ...
  const sources = [
    {
      name: "Upstox API (Primary)",
      icon: <Wifi className="h-4 w-4" />,
      status: upstoxStatus === "success" ? "online" : upstoxStatus === "error" ? "offline" : health?.sources?.upstox ? "online" : "unknown",
      detail: upstoxStatus === "success"
        ? "Primary source · Option Chain, Greeks, Live LTP"
        : health?.sources?.upstox
        ? "Credentials loaded from .env · Option Chain, Expiry, Live Feed"
        : upstoxMessage || "Click Test to verify — provides Option Chain, Greeks, Live Ticks",
      color: upstoxStatus === "success" || health?.sources?.upstox ? "text-emerald-500" : upstoxStatus === "error" ? "text-red-500" : "text-zinc-500",
    },
    {
      name: "Upstox Live Feed",
      icon: <Zap className="h-4 w-4" />,
      status: wsConnected ? "online" : "offline",
      detail: wsConnected
        ? `Live LTP · ${health?.websocket?.cachedTicks || 0} cached, ${health?.websocket?.instrumentsSubscribed || 0} instruments`
        : "Requires Upstox access token · Real-time index + VIX ticks",
    ...
    },
    {
      ...
      detail: "Fallback · Indices, Sectors, A/D, Option Chain if Upstox fails",
      ...
      <CardContent className="space-y-2">
        {sources.map((src) => (
          <div key={src.name} className="flex items-center gap-3 p-2 rounded-md bg-accent/20">
            ...
            {src.name.startsWith("Upstox API") && (
              <Button
                size="sm"
                variant="outline"
                className="h-7 text-xs gap-1"
                onClick={handleTestUpstox}
                disabled={upstoxStatus === "testing"}
              >
                {upstoxStatus === "testing" ? (
                  <><Loader2 className="h-3 w-3 animate-spin" /> Testing</>
                ) : upstoxStatus === "success" ? (
