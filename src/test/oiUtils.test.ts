...
describe("computeOiSnapshotMetrics", () => {
  it("tracks PCR delta between snapshots", () => {
    const key = `test-${Date.now()}`;
    const chain1 = [makeRow(24000, 100000, 150000)];
    const first = computeOiSnapshotMetrics(chain1, key);
    expect(first.pcrDelta).toBeNull();

    const chain2 = [makeRow(24000, 90000, 180000)];
    const second = computeOiSnapshotMetrics(chain2, key);
    expect(second.priorPcrOI).toBe(first.pcrOI);
    expect(second.pcrDelta).not.toBeNull();
  });
});
