...
import { computeOiSnapshotMetrics, getMaxPain } from "@/lib/oiUtils";
import {
  INDEX_OI_INSIGHTS_CHAIN_POLL_MS,
  INDEX_OI_INSIGHTS_CHAIN_STALE_MS,
  OI_CHAIN_AFTER_HOURS_POLL_MS,
  OI_CHAIN_POLL_MS,
  OI_CHAIN_RETRY_POLL_MS,
  OI_CHAIN_STALE_MS,
  PRICE_WS_CONNECTED_INDEX_POLL_MS,
  PRICE_WS_FALLBACK_POLL_MS,
...
import { isSpotSourceDebugEnabled, recordIndicesFetch } from "@/lib/spotSourceDebug";
...
export function useLiveIndices() {
  ...
  const hasFreshIndexTicks = useAnyFreshIndexTick();
  ...
  const pollingQuery = useQuery({
    ...
    queryFn: async () => {
      if (shouldTryProxy()) {
        try {
          ...
          if (data && data.length > 0) {
            markProxyOnline();
            if (isSpotSourceDebugEnabled()) recordIndicesFetch(true);
            return { data, isLive: true, fetchedAt: Date.now() };
          }
          if (isSpotSourceDebugEnabled()) recordIndicesFetch(false);
      ...
      if (dbIndices.length > 0) {
        return { data: dbIndices, isLive: false, source: "database" as const, fetchedAt: Date.now() };
      }
      // NO MOCK — return empty
      return { data: [] as IndexData[], isLive: false, fetchedAt: 0 };
    },
    refetchInterval: hasFreshIndexTicks
      ? PRICE_WS_CONNECTED_INDEX_POLL_MS
      : (query) => (query.state.data?.isLive ? PRICE_WS_FALLBACK_POLL_MS : 60000),
    staleTime: 5000,
    retry: 1,
  });
  ...
  const mergedData = useMemo(() => {
    if (wsIndices.length > 0) {
      ...
      return { data: merged, isLive: true, fetchedAt: pollingQuery.data?.fetchedAt ?? Date.now() };
    }

    return pollingQuery.data || { data: [] as IndexData[], isLive: false, fetchedAt: 0 };
...
export type LiveOptionChainData = {
  chain: OptionData[];
  spotPrice: number;
  /** Spot from OI/chain REST before WebSocket merge */
  chainSpotPrice?: number;
  spotChange?: number;
  spotChangePercent?: number;
  spotIsLive?: boolean;
  spotSource?: "websocket" | "indices" | "chain";
  expiries: ExpiryDate[];
  lotSize: number;
  stepSize: number;
  maxPain: number;
  ...
  oiMetrics: ReturnType<typeof computeOiSnapshotMetrics> | null;
  oiPollIntervalMs: number;
};

export type LiveOptionChainOptions = {
  /** Faster poll on Index OI Insights (still ~NSE OI cadence, not tick data). */
  pollIntervalMs?: number;
  staleTimeMs?: number;
};

// ── Hook: Live Option Chain (OI / PCR / Greeks) ──
// OI polls ~3 min; index spot LTP merged from WebSocket (~2s via proxy).
export function useLiveOptionChain(symbol: string, expiry?: string, options?: LiveOptionChainOptions) {
  const oiCacheKey = `${symbol}:${expiry ?? "nearest"}`;
  const pollMs = options?.pollIntervalMs ?? OI_CHAIN_POLL_MS;
  const staleMs = options?.staleTimeMs ?? OI_CHAIN_STALE_MS;
  ...
  const query = useQuery({
        ...
        try {
          ...
          if (result) {
            ...
            if (hasChainData || isAfterHours) {
              const oiMetrics = hasChainData
                ? computeOiSnapshotMetrics(result.chain, oiCacheKey)
                : null;
              return {
    ...
    refetchInterval: (query) => {
      ...
      if (data?.isLive) return pollMs;
      if (data?.afterHours) return OI_CHAIN_AFTER_HOURS_POLL_MS;
      return OI_CHAIN_RETRY_POLL_MS;
    },
    staleTime: staleMs,
    retry: 1,
  });

  const chainSpot = query.data?.spotPrice ?? 0;
  const realtimeSpot = useIndexRealtimeSpot(symbol, chainSpot);

  const data = useMemo((): LiveOptionChainData | null | undefined => {
    if (!query.data) return query.data;
    const spot = realtimeSpot.spotPrice > 0 ? realtimeSpot.spotPrice : query.data.spotPrice;
    return {
      ...query.data,
      chainSpotPrice: query.data.spotPrice,
      spotPrice: spot,
      spotChange: realtimeSpot.change,
      spotChangePercent: realtimeSpot.changePercent,
      spotIsLive: realtimeSpot.isLive,
      spotSource: realtimeSpot.source,
    };
  }, [query.data, realtimeSpot]);
